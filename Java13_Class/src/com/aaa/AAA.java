...
// 그래서 ()안의 내용을 사용하지 않고 있다.

public class AAA {
	
	String color; 				  // instance 변수 (non-static) (자동차 마다 다른색깔)
	int wheel;					  // instance 변수 (non-static) (자동차 마다 다른 바퀴 개수)
	static boolean engine = true; // class 변수 (자동차 전체 공통 적용)
...
class Test{
 ...
 명시된 생성자를 선언하게 되면 명시 생성자만 호출 되고 기본생성자는 제공되지 않는다.
