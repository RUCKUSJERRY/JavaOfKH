...
public class MTest02 {
	...
	public static void main(String[] args) {
		...
		set.add(s2);
		set.add(s3);
		set.add(new Score("이용준", 100, 100, 100));
	...
	public static void printCollection(Set<Score> set) {
		...
		for (int i = 0; i < oArr.length; i++) {
			...
			// Object type이라서 Score타입의 메소드를 못불러온다. 오브젝트가 부모니까
			// 그래서 Score로 형변환을 해준다음에 메소드를 불러온다.
			System.out.println(((Score) oArr[i]).getName() + "\t" + ((Score) oArr[i]).getAvg());
		}
		System.out.println("=====================================================");
		// 3. Iterator : 컬렉션 저장 요소를 읽어오는 표준화된 방법 / map은 이터레이터 쓸수 없다.
